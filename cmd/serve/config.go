...
func buildConfig(addr string) (cfg *config.Config, cfgPath string, err error) {
	...
	cfg = &config.Config{
		...
		SDKConfig: config.SDKConfig{
			APIKeys: nil,
			// ponytail: heartbeat keeps the client SSE connection alive while upstream
			// NVIDIA predict is mid-reasoning (gaps of tens of seconds between deltas).
			// The SDK default is 0 = off, which surfaces as "The response stopped arriving"
			// when the client's idle timeout fires first.
			Streaming: config.StreamingConfig{KeepAliveSeconds: 15},
		},
	}
