...
## Windows 一键运行（run.ps1，可选 headless-shell）

`run.ps1` 封装了构建/后台/开机自启，并可选 **Chrome for Testing headless-shell**（比完整 Chrome 省 ~30% 内存，机器无需预装 Chrome）。

```powershell
# 0) 下载 headless-shell 到 bin/（仅需一次；幂等，已存在则跳过）
.\run.ps1 -InstallChrome

# 1) 前台运行（会自动检测 bin/ 下的 headless-shell 并设 CHROME_PATH）
.\run.ps1

# 2) 后台运行（无窗口，日志写 nvpi.log）
.\run.ps1 -Background

# 3) 开机自启：在 Windows 启动目录建 .lnk（隐藏窗口 + 后台）
.\run.ps1 -InstallStartup

# 停止
.\run.ps1 -Stop
```

`CHROME_PATH` 解析优先级：`-ChromePathParam` > 已有 `$env:CHROME_PATH` > `bin/chrome-headless-shell.exe.txt`（由 `-InstallChrome` 写入）。若三者皆空，chromedp 回退到系统 Chrome 并给出 warning。

> **English:** `run.ps1` wraps build/background/startup and optionally installs **Chrome for Testing headless-shell** (~30% less RAM than full Chrome, no system Chrome required). `CHROME_PATH` resolution: `-ChromePathParam` > existing `$env:CHROME_PATH` > `bin/chrome-headless-shell.exe.txt` (written by `-InstallChrome`). If all empty, chromedp falls back to system Chrome with a warning.
